...
CREATE TYPE "public"."order_status" AS ENUM('pending', 'payment-processing', 'paid', 'failed', 'expired', 'canceled');--> statement-breakpoint
...
CREATE TABLE "chat_rooms" (
	...
	CONSTRAINT "chat_rooms_product_variant_pair_ck" CHECK (("chat_rooms"."product_id" is null and "chat_rooms"."product_variant_id" is null) or ("chat_rooms"."product_id" is not null and "chat_rooms"."product_variant_id" is not null))
);
--> statement-breakpoint
...
CREATE TABLE "merchant_trades" (
	...
	"variant_ids" uuid[] DEFAULT '{}' NOT NULL,
	"cvs_store_info" jsonb,
	"shipping_cost" double precision DEFAULT 0,
	"shipping_cost_deduction" double precision DEFAULT 0,
	CONSTRAINT "merchant_trades_product_variant_length_ck" CHECK (cardinality("merchant_trades"."product_ids") = cardinality("merchant_trades"."variant_ids"))
);
--> statement-breakpoint
CREATE TABLE "order_items" (
	...
	"variant_name_at_sale" text,
	"variant_sku_at_sale" text,
	"variant_option_values_at_sale" jsonb,
...
CREATE TABLE "orders" (
	...
	"order_payment" "order_payment" DEFAULT 'Credit' NOT NULL,
	"complete_email_sent" boolean DEFAULT false NOT NULL,
...
CREATE TABLE "product_variants" (
	...
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL,
	CONSTRAINT "product_variants_stock_non_negative_ck" CHECK ("product_variants"."stock" >= 0),
	CONSTRAINT "product_variants_reserve_non_negative_ck" CHECK ("product_variants"."reserve" >= 0),
	CONSTRAINT "product_variants_reserve_not_exceed_stock_ck" CHECK ("product_variants"."reserve" <= "product_variants"."stock")
);
--> statement-breakpoint
...
CREATE TABLE "users" (
	...
	"bank_code" text,
	"bank_account" text,
	"bank_account_updated_at" timestamp with time zone,
	"created_at" timestamp with time zone DEFAULT now() NOT NULL,
	"updated_at" timestamp with time zone DEFAULT now() NOT NULL
);
--> statement-breakpoint
...
CREATE INDEX "refund_logs_refund_item_id_created_at_idx" ON "refund_logs" USING btree ("refund_item_id","created_at");--> statement-breakpoint
