...
CREATE TYPE "public"."user_coin_transaction_type" AS ENUM('acquire', 'reserve', 'spend', 'reversal', 'refund', 'expiry', 'manual');--> statement-breakpoint
...
CREATE UNIQUE INDEX "advertisement_transactions_idempotency_key_uk" ON "advertisement_transactions" USING btree ("idempotency_key") WHERE "advertisement_transactions"."idempotency_key" is not null;